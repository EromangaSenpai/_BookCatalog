...
if(!isset($_COOKIE['id']))
    header('Location: ../UpdatePages/Choose_Publish.php');

if (!R::testConnection())
    exit ('Нет соединения с базой данных');

$info = '';
$publish = R::findOne('publish','id = ?', array($_COOKIE['id']));

$past_name = $publish->name;
$past_address = $publish->address;
$past_telephone = $publish->telephone;
$past_date = $publish->date;

if($_SERVER['REQUEST_METHOD'] == 'POST')
{

        $publish->name = $_POST['publishName'];
        $publish->address = $_POST['publishAddress'];
        $publish->telephone = $_POST['tel'];
        $publish->date = $_POST['date'];
        R::store($publish);

        unset($past_name);
        unset($past_address);
        unset($past_telephone);
        unset($past_date);
    ...
    <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
        ...
        <div class="row form-group">
            ...
            <div class="col-sm-6">
                ...
                <input value="<?php echo @$past_name?>" type="text" maxlength="50" name="publishName" class="form-control" id="publishName" placeholder="Publisher name" required>
            </div>
            <div class="col-sm-6">
                <label for="publishAddress" class="text-light">Publishing Address:</label>
                <input value="<?php echo @$past_address?>" type="text" maxlength="60" name="publishAddress" class="form-control" id="publishAddress" placeholder="Publishing address" required>
            </div>
        ...
        <div class="row form-group">
            ...
            <div class="col-sm-6">
                ...
                <input value="<?php echo @$past_telephone?>" type="tel" name="tel" class="form-control" id="product" placeholder="Publisher's phone" required>
            </div>
            <div class="col-sm-6">
                <label for="date" class="text-light">Date Of Publishing:</label>
                <input value="<?php echo @$past_date?>" type="date" name="date" class="form-control" id="date" placeholder="Date of publishing" required>
            </div>
